...
module.exports = {
	...
	async execute(message) {
		...
		/**
		 * @type {String[]}
		 * @description Name and parameters of the procedure received from the database.
		 */
		...
		const procedureParams = data && (await data.Procedure.split(" "));
		...
		const command =
			...
			client.customCommands.get(commandName) ||
			client.customCommands.find(
				(cmd) => cmd.aliases && cmd.aliases.includes(commandName)
			);
		...
		try {
			!procedureParams
				...
				: command.execute(message, args, procedureParams, commandName);
		} catch (error) {
			console.error(error);
			message.reply({
