...
const CustomCommand = require("./lib/custom-commands/customCommand");
const TimedMessage = require("./lib/timed-job/timedMessage");
...
/**********************************************************************/
// Define Collection of Commands, Slash Commands, Settings and cooldowns
...
client.timedMessages = new Collection();
...
/**
 * @type {String[]}
 * @description All avaliable settings.
 */
(
	async () => {
		const settings = await getKnexConnection().select().from("Settings");
		// Loop through all settings and store them in settings collection.
		for (const setting of settings) {
			client.settings.set(setting.setting_name, setting);
		}
	}
)();
...
/**********************************************************************/
// Registration of Message-Based Custom Commands.

/**
 * @type {String[]}
 * @description All command rows from the database.
 */

// Loop through all rows and store commands in customCommands collection.
(
	async () => {
		try {
			const customCommands = await getKnexConnection()
				.select()
				.from("CustomCommands");

			for (const commandData of customCommands) {
				const command = new CustomCommand(
					...Object.values(delete commandData.ID && commandData)
				);
...
/**********************************************************************/
// Registration of Timed-Meesage.

/**
 * @type {String[]}
 * @description All Timed-Meesage rows from the database.
 */

// Loop through all rows and store them in timedMessages collection.
(
	async () => {
		try {
			const timedMessages = await getKnexConnection()
				.select(
					"TimedMessages.ID",
					"Title as title",
					"Message as message",
				...
				.from("TimedMessages")
				.join(
					"TimedMessagesConfiguration",
					"TimedMessages.ConfigurationID",
					"TimedMessagesConfiguration.ID"
				);

			for (const messageData of timedMessages) {
				const message = new TimedMessage(...Object.values(messageData));
				client.timedMessages.set(messageData.ID, message);
			}
			// console.log(client.timedMessages);
		} catch (error) {
			console.error(error);
		}
	}
